...
module.exports = function(Contractorprofile) {
        ...
        UserModel.findOne({ where: { email: contractorObjData.contractor_email_id } }, function(err, userResult) {
            if (err) {
                ...
                if (userResult != null && userResult != undefined && userResult != '') {
                    ...
                    cb(error(406, "user result already exist" + userResult), null);
                } else {
                    // var passwordData;
    ...
    Contractorprofile.edit_contractor = function(contractorObj, cb) {
        ...
        var userFindquery = "select usr.username,usr.email from [User] as usr Inner join contractor_profile as contractor on contractor.contractor_email_id = usr.email where contractor.contractor_id =" + contractorObjData.contractor_id;
