...
var EditEquipmentComponent = /** @class */ (function () {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                if (equipmentForm.valid) {
                    ...
                    this.checkCount(this.editEquipment).then(function (total) {
                        if (total > 0 && !_this.editEquipment.status) {
                            _this.loading = false;
                            _this.result = "Equipment is already been used so it cannot be inactive";
                            _this.display = true;
                        }
                        else {
                            _this.equipmentApi.upsertPatch(_this.editEquipment).subscribe(function (data) {
                                _this.loading = false;
    ...
    EditEquipmentComponent.prototype.checkCount = function (dataObj) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            try {
                var access_token = _this.loopAuth.getAccessTokenId();
                var equipment_id = dataObj.equipment_id;
                var cond = { "equipment_id": equipment_id };
                var api = "/api/equipments_mappings/count?where=";
                var url = hostname + api + encodeURI(JSON.stringify(cond)) + "&access_token=" + access_token;
                _this.http.get(url).subscribe(function (data) {
                    //console.log("Count ",data.json());
                    resolve(data.json().count);
