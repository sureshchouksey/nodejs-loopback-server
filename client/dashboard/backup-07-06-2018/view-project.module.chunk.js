...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6_angular2_lightbox__ = __webpack_require__("./node_modules/angular2-lightbox/index.js");
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7_angular2_cookie_core__ = __webpack_require__("./node_modules/angular2-cookie/core.js");
...
var ViewProjectComponent = /** @class */ (function () {
    function ViewProjectComponent(router, http, projectApi, photosApi, _cookieService, _lightbox, confirmationService, appLoginApi, loopAuth, route) {
        this.router = router;
        this.http = http;
    ...
    ViewProjectComponent.prototype.ngOnInit = function () {
        ...
        this.loading = true;
        this.projectApi.getProjects(dataArrObj).subscribe(function (projectData) {
            ...
            if (data.result.length > 0) {
                ...
                if (data.result[0].created_date) {
                    data.result[0].created_date = __WEBPACK_IMPORTED_MODULE_4_moment__(data.result[0].created_date).format("DD - MMM - YYYY");
                }
                else {
                    data.result[0].created_date = "";
                }
                if (data.result[0].updated_date) {
                    data.result[0].updated_date = __WEBPACK_IMPORTED_MODULE_4_moment__(data.result[0].updated_date).format("DD - MMM - YYYY");
                }
                else {
                    data.result[0].updated_date = "";
                }
                if (data.result[0].project_start_date) {
                    data.result[0].project_start_date = __WEBPACK_IMPORTED_MODULE_4_moment__(data.result[0].project_start_date).format("DD - MMM - YYYY");
                }
                if (data.result[0].project_completion_date) {
                    data.result[0].project_completion_date = __WEBPACK_IMPORTED_MODULE_4_moment__(data.result[0].project_completion_date).format("DD - MMM - YYYY");
                }
                if (data.result[0].project_cover_photo) {
                    var img = data.result[0].project_cover_photo;
                    //------- this code is commented because of duplication of cover photo in image table and project table
                    // data.result[0].project_cover_photo = hostname+"/api/containers/projectImage/download/"+img+'?access_token='+this.loginFlagToken;
                    // this._albums.push(
                    // 	{
                ...
                _this.projectData.sub_type_of_housing = (projectResult.sub_type_of_housing) ? projectResult.sub_type_of_housing : "";
            ...
            // 	this.workGroupArr = [];
            // 	let wrkGrpLength = projectResult.work_group.length;
            // 	let wrkGrpEmptyCount = 0;
            // 	for (const key in projectResult.work_group) {
            // 		let wrkGrpType = projectResult.work_group[key].member_type;
            // 			let wrkGropNamesArr = JSON.parse(projectResult.work_group[key].member_name);
            // 			let wrkGroupNamesArr = [];
            // 			if(wrkGropNamesArr.length == 0){
            // 				wrkGrpEmptyCount++;
            // 			}
            // 			for(let i = 0;i<5;i++){
            ...
            // 			}
            // 			this.workGroupArr.push(wrkGrpObj);
            // 	}
            //for (const key in projectResult.work_group) {
            if (projectResult.work_group.length > 0) {
                /*let wrkGropNamesArr1:any[];
                let wrkGropNamesArr2:any[];
                let wrkGropNamesArr3:any[];
                let wrkGropNamesArr4:any[];
                for (const key in projectResult.work_group) {
                    if (projectResult.work_group[key].member_type == "architect") {
                        wrkGropNamesArr1 = JSON.parse(projectResult.work_group[key].member_name);
                        
                    }
                    if (projectResult.work_group[key].member_type == "engineer") {
                        wrkGropNamesArr2 = JSON.parse(projectResult.work_group[key].member_name);
                        
                    }
                    if (projectResult.work_group[key].member_type == "designer") {
                        wrkGropNamesArr3 = JSON.parse(projectResult.work_group[key].member_name);
                        
                    }
                    if (projectResult.work_group[key].member_type == "manager") {
                        wrkGropNamesArr4 = JSON.parse(projectResult.work_group[key].member_name);
                ...
                for(let key = 0; key < 5;key++){
                    let wrkGroupNamesArr = [];
                    if(wrkGropNamesArr1[key].name != undefined && wrkGropNamesArr1[key].name != null && wrkGropNamesArr1[key].name != ''){
                        wrkGroupNamesArr.push(wrkGropNamesArr1[key].name);
                    }else{
                        wrkGroupNamesArr.push({'name':'-'});
                    }
                    if(wrkGropNamesArr2[key].name != undefined && wrkGropNamesArr2[key].name != null && wrkGropNamesArr2[key].name != ''){
                        wrkGroupNamesArr.push(wrkGropNamesArr2[key].name);
                    }else{
                        wrkGroupNamesArr.push({'name':'-'});
                    }
                    if(wrkGropNamesArr3[key].name != undefined && wrkGropNamesArr3[key].name != null && wrkGropNamesArr3[key].name != ''){
                        wrkGroupNamesArr.push(wrkGropNamesArr3[key].name);
                    }else{
                        wrkGroupNamesArr.push({'name':'-'});
                    }
                    if(wrkGropNamesArr4[key].name != undefined && wrkGropNamesArr4[key].name != null && wrkGropNamesArr4[key].name != ''){
                        wrkGroupNamesArr.push(wrkGropNamesArr4[key].name);
                    }else{
                        wrkGroupNamesArr.push({'name':'-'});
                    }
                    ...
                    this.workGroupArr[key] = (wrkGrpObj);
                }*/
                for (var key in projectResult.work_group) {
                    var wrkGrpType = projectResult.work_group[key].member_type;
                    var wrkGropNamesArr = JSON.parse(projectResult.work_group[key].member_name);
                ...
                console.log("wrkGrpObj", _this.workGroupArr);
                if (_this.workGroupArr.length == 0) {
                    _this.showWrkGroup = false;
                }
            }
        }, function (err) {
            _this.loading = false;
    ...
    ViewProjectComponent.prototype.getProjectPhotos = function () {
        var _this = this;
        var cond = { "where": { "project_id": this.id, "status": 1 } };
        var api = "/api/project_photos?filter=";
        var url = hostname + api + encodeURI(JSON.stringify(cond)) + "&access_token=" + this.loginFlagToken;
        //this.photosApi.find({where:{and: [{"project_id":this.id}, {"status":1}]}})
            ...
            if (data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                    if (data[i]['project_photo_source']) {
                        ...
                        //img = "http://phapi.experiencecommerce.com:4500/api/containers/projectImage/download/"+img+'?access_token='+this.loginFlagToken;
                        img = hostname + "/api/containers/projectImage/download/" + img + '?access_token=' + _this.loginFlagToken;
                        _this._albums.push({
                            src: img,
                            thumb: img
                        });
    ...
    ViewProjectComponent = __decorate([
        ...
        __metadata("design:paramtypes", [__WEBPACK_IMPORTED_MODULE_1__angular_router__["Router"], __WEBPACK_IMPORTED_MODULE_3__angular_http__["a" /* Http */], __WEBPACK_IMPORTED_MODULE_2__shared_loopback_sdk__["q" /* ProjectsApi */], __WEBPACK_IMPORTED_MODULE_2__shared_loopback_sdk__["p" /* Project_photosApi */], __WEBPACK_IMPORTED_MODULE_7_angular2_cookie_core__["CookieService"], __WEBPACK_IMPORTED_MODULE_6_angular2_lightbox__["Lightbox"], __WEBPACK_IMPORTED_MODULE_5_primeng_primeng__["ConfirmationService"], __WEBPACK_IMPORTED_MODULE_2__shared_loopback_sdk__["b" /* App_loginApi */], __WEBPACK_IMPORTED_MODULE_2__shared_loopback_sdk__["m" /* LoopBackAuth */], __WEBPACK_IMPORTED_MODULE_1__angular_router__["ActivatedRoute"]])
