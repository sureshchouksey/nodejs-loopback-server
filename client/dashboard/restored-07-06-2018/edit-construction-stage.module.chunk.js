...
var EditConstructionStageComponent = /** @class */ (function () {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                ...
                if (constrctnStgForm.valid) {
                    ...
                    this.checkCount(this.editConstnStg).then(function (total) {
                        if (total > 0 && !_this.editConstnStg.status) {
                            _this.loading = false;
                            _this.result = "Construction-Stage is already been used so it cannot be inactive";
                            _this.display = true;
                        }
                        else {
                            _this.constStgApi.upsertPatch(_this.editConstnStg).subscribe(function (data) {
                                _this.loading = false;
    ...
    EditConstructionStageComponent.prototype.checkCount = function (dataObj) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            try {
                var access_token = _this.loopAuth.getAccessTokenId();
                var constrctStgId = dataObj.construction_stage_id;
                var cond = { "construction_stage_id": constrctStgId };
                var api = "/api/construction_stage_mappings/count?where=";
                var url = hostname + api + encodeURI(JSON.stringify(cond)) + "&access_token=" + access_token;
                _this.http.get(url).subscribe(function (data) {
                    //console.log("Count ",data.json());
                    resolve(data.json().count);
