...
var EditSpecializationComponent = /** @class */ (function () {
                ...
                if (specializationForm.valid) {
                    ...
                    this.ifNameExist(this.editSpecialization).then(function (total) {
                        if (total > 0) {
                            ...
                            _this.checkCount(_this.editSpecialization).then(function (total) {
                                if (total > 0 && !_this.editSpecialization.status) {
                                    _this.loading = false;
                                    _this.result = "Specialization is already been used so it cannot be inactive";
                                    _this.display = true;
                                }
                                else {
    ...
    EditSpecializationComponent.prototype.checkCount = function (dataObj) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            try {
                var access_token = _this.loopAuth.getAccessTokenId();
                var specialization_id = dataObj.specialization_id;
                var cond = { "specialization_id": specialization_id };
                var api = "/api/contractor_specialization_mappings/count?where=";
